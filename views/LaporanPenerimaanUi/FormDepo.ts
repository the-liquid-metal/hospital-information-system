 ...
 *
 * @see http://localhost/ori-source/fatma-pharmacy/views/laporan/laporanpenerimaan_rekap.php the original file
 * last exist of tlm\his\FatmaPharmacy\views\LaporanUi\LaporanPenerimaanRekap: commit-e37d34f4
 */
final class FormDepo
{
    private string $output;

    public function __construct(string $registerId, string $actionUrl, string $depoPenerimaSelect, string $depoAsalSelect)
    {
...
note: different expression compared to original
<!--suppress TypeScriptUnresolvedVariable -->
<script type="text/tsx">
namespace his.FatmaPharmacy.views.LaporanPenerimaanUi.FormDepo {
    export interface Fields {
        idDepoPenerima:  string;
        idDepoAsal:      string;
        tanggalAwal:     string;
        tanggalAkhir:    string;
    }
...
tlm.app.registerModule(class extends spa.BaseModule {
    ...
    _structure = {
        ...
        form: {
            class: ".laporanDepoFrm",
            row_1: {
                box: {
                    title: tlm.stringRegistry._<?= $h("Parameter") ?>,
                    formGroup_1: {
                        label: tlm.stringRegistry._<?= $h("Depo Penerima") ?>,
                        select: {class: ".idDepoPenerimaFld", name: "idDepoPenerima"}
                    },
                    formGroup_2: {
    ...
    constructor(divElm) {
        ...
        /** @type {HTMLSelectElement} */ const idDepoPenerimaFld = divElm.querySelector(".idDepoPenerimaFld");
        ...
        tlm.app.registerSelect("_<?= $depoPenerimaSelect ?>", idDepoPenerimaFld);
        tlm.app.registerSelect("_<?= $depoAsalSelect ?>", idDepoAsalFld);
        this._selects.push(idDepoPenerimaFld, idDepoAsalFld);

        const laporanDepoWgt = new spl.AjaxFormWidget({
            element: divElm.querySelector(".laporanDepoFrm"),
            /** @param {his.FatmaPharmacy.views.LaporanPenerimaanUi.FormDepo.Fields} data */
            loadData(data) {
                idDepoPenerimaFld.value = data.idDepoPenerima ?? "";
        ...
        this._widgets.push(laporanDepoWgt, tanggalAwalWgt, tanggalAkhirWgt);
        tlm.app.registerWidget(this.constructor.widgetName, laporanDepoWgt);
    }
});
</script>
