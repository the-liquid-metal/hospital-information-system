...
namespace his.FatmaPharmacy.views.KelompokBarangUi.Table {
...
tlm.app.registerModule(class extends spa.BaseModule {
    ...
    _structure = {
        row_1: {
            widthColumn: {
                heading3: {text: tlm.stringRegistry._<?= $h("Daftar Kelompok Barang") ?>}
            }
        },
        row_2: {
            widthColumn: {
            ...
            widthTable: {
                ...
                thead: {
                    tr: {
                        ...
                        td_2: {text: tlm.stringRegistry._<?= $h("Kelompok Barang") ?>},
    ...
    constructor(divElm) {
        ...
        /** @type {HTMLInputElement} */ const kelompokBarangFld = divElm.querySelector(".kelompokBarangFld");

        const saringWgt = new spl.StaticFormWidget({
            element: divElm.querySelector(".saringFrm"),
            /** @param {his.FatmaPharmacy.views.KelompokBarangUi.Table.FormFields} data */
            loadData(data) {
                kodeFld.value = data.kode ?? "";
                kelompokBarangFld.value = data.kelompokBarang ?? "";
            },
            submit() {
                itemWgt.refresh({
                    query: {kode: kodeFld.value, kelompokBarang: kelompokBarangFld.value}
                });
            }
        });
