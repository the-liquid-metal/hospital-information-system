...
namespace tlm\his\FatmaPharmacy\views\KelompokBarangUi;
 ...
 *
 * @see http://localhost/ori-source/fatma-pharmacy/views/farmasi/Kelompokbarang/add.php the original file
 */
final class Form
{
...
namespace his.FatmaPharmacy.views.KelompokBarangUi.Form {
...
tlm.app.registerModule(class extends spa.BaseModule {
            ...
            row_1: {
                box: {
                    ...
                    formGroup_2: {
                        ...
                        input: {class: ".namaFld", name: "kelompokBarang"}
                    }
                }
            },
            row_2: {
    ...
    constructor(divElm) {
        ...
        const kelompokBarangWgt = new spl.AjaxFormWidget({
            element: divElm.querySelector(".kelompokBarangFrm"),
            /** @param {his.FatmaPharmacy.views.KelompokBarangUi.Form.FormFields} data */
            loadData(data) {
                idFld.value = data.id ?? "";
                actionFld.value = data.action ?? "";
                kodeWgt.value = data.kode ?? "";
                namaWgt.value = data.kelompokBarang ?? "";
            },
            ...
            profile: {
                add() {
                    ...
                    this.loadData({});
                    formTitleTxt.innerHTML = str._<?= $h("Tambah Kelompok Barang") ?>;
                },
                edit(data, fromServer) {
                    this._actionUrl = "<?= $editActionUrl ?>";
                    this.loadData(data, fromServer);
                    formTitleTxt.innerHTML = str._<?= $h("Ubah Kelompok Barang") ?>;
                },
            },
            onInit() {
        ...
        const namaWgt = new spl.AjaxInputWidget({
            ...
            url: "<?= $cekUnikNamaUrl ?>",
            term: "value",
            additionalData: {field: "kelompok_barang"}
        });

        this._element = divElm;
        divElm.moduleWidget = this;
        this._widgets.push(kelompokBarangWgt, kodeWgt, namaWgt);
        tlm.app.registerWidget(this.constructor.widgetName, kelompokBarangWgt);
    }
});
</script>
