 ...
 *
 * @see http://localhost/ori-source/fatma-pharmacy/views/eresepdepodr/listresep.php the original file
 */
final class TableResep
{
    ...
    public function __construct(
        ...
        string $viewWidgetId,
        string $resepIghWidgetId,
        string $resepDokterWidgetId,
        string $depoSelect,
    ) {
...
namespace his.FatmaPharmacy.views.EresepDepoDrUi.ListResep {
...
tlm.app.registerModule(class extends spa.BaseModule {
    ...
    _structure = {
        row_1: {
            widthColumn: {
                heading3: {text: tlm.stringRegistry._<?= $h("Daftar Resep Dokter") ?>}
            }
        },
        row_2: {
            widthColumn: {
    ...
    constructor(divElm) {
        ...
        this._widgets.push(idDepoFld);

        const saringWgt = new spl.StaticFormWidget({
            element: divElm.querySelector(".saringFrm"),
            /** @param {his.FatmaPharmacy.views.EresepDepoDrUi.ListResep.FormFields} data */
            loadData(data) {
        ...
        const itemWgt = new spl.TableWidget({
            ...
            columns: {
                ...
                5: {field: "namaPasien"},
                6: {field: "tanggalResep", formatter: tlm.dateFormatter},
                7: {field: "totalCeil", formatter: tlm.intFormatter},
                8: {formatter(unused, item) {
                    const {noResep: value, verifikasi, tanggalVerifikasi, transfer, jenisResep} = item;
                    const disabled = (transfer || jenisResep == "Pembelian Bebas" || jenisResep == "Pembelian Langsung");
                    const type = disabled ? "warning" : "success";

                    const editBtn =        draw({class: ".editBtn",        value, text: str._<?= $h("Edit") ?>});
                    const resepDokterBtn = draw({class: ".resepDokterBtn", value, text: str._<?= $h("Resep Dokter") ?>});
                    const resepIghBtn =    draw({class: ".resepIghBtn",    value, text: str._<?= $h("Resep IGH") ?>});
                    const viewBtn =        draw({class: ".viewBtn",        value, text: str._<?= $h("Lihat") ?>});
                    const returBtn =       draw({class: ".returBtn",       value, text: str._<?= $h("Retur") ?>});
                    const deleteBtn =      draw({class: ".deleteBtn",      value, text: str._<?= $h("Delete") ?>});
                    const transferBtn =    draw({class: ".transferBtn",    value, text: str._<?= $h("Transfer") ?>, type, disabled});
                    const verifikasiBtn =  draw({class: ".verifikasiBtn",  value, text: str._<?= $h("Verifikasi") ?>});

                    return editBtn + resepDokterBtn + resepIghBtn + viewBtn + transferBtn +
        ...
        itemWgt.addDelegateListener("tbody", "click", (event) => {
            const resepIghBtn = event.target;
            if (!resepIghBtn.matches(".resepIghBtn")) return;

            const widget = tlm.app.getWidget("_<?= $resepIghWidgetId ?>");
            widget.show();
            widget.loadData({noResep: resepIghBtn.value}, true);
        });

        itemWgt.addDelegateListener("tbody", "click", (event) => {
            const resepDokterBtn = event.target;
            if (!resepDokterBtn.matches(".resepDokterBtn")) return;

            const widget = tlm.app.getWidget("_<?= $resepDokterWidgetId ?>");
            widget.show();
            widget.loadData({noResep: resepDokterBtn.value}, true);
        });
